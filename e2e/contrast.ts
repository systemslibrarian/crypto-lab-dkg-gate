 ...
 *    `color-mix()`. Every surface this lab uses to say something is one:
 *    `.cell-ok` and `.cell-bad` in the share matrix, `.banner-ok`,
 *    `.banner-bad` and `.banner-warn`, `.party-disq`, `.phase-nav
 *    li.phase-done`, `.row-chosen`, `.chip`, `.chip-pk`, `#app .btn-primary`
 *    and `.cl-hero-why` are each a `color-mix(in oklab, …, transparent)` over
 *    `--panel` or `--panel-2`. axe files all of them under `incomplete`, which
 *    means a violations-only gate measured the contrast of almost nothing this
 *    demo actually renders as a verdict.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. Here `.cl-hero-sub` sits at
 *    `.85`, and `.chip` ships at `opacity: 0` and is animated to 1 (see the
 *    note in `gate.ts` on why that is the interesting case rather than a
 *    nuisance).
 *
 ...
 *    boxes are the `.scroll-region` wrappers around the share-delivery matrix,
 *    the adversary's round-1 view and the candidate-key table — all
 *    `overflow: auto` around `white-space: nowrap` content wider than the box
 *    at any narrow width. Content scrolled past them is
 ...
 *    marks, both `aria-hidden` and glyph-free; every figure it draws (the party
 *    cards, the share matrix, the chip sum) is HTML. So the sibling walk finds
 ...
 * one — nor is it one to axe's `color-contrast` rule. This page has exactly one
 * such glyph, `.select-wrap::after { content: '▾' }`, which is the entire
 * dropdown affordance for five `appearance: none` selects. It was measured by
 * hand from real screenshot pixels instead: 6.75:1 in dark and 6.41:1 in light
 * against the `--panel-2` fill it sits on, clearing both the 4.5:1 a 14.4px
 * glyph needs and the 3:1 of SC 1.4.11.
 */
 ...
 * uses. Nothing here is transient: every exhibit computes synchronously on the
 * click (real ristretto255 math, but a whole ceremony is under 100ms) and then
 * stays on screen until something else is clicked, so there is no styling with
 * a lifetime shorter than a scan. The parameter is
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven scan here walks a 5x5 share matrix whose every cell carries a
     * code element and a two-span mark, a sixteen-row candidate-key table, the
     * per-party final-share list, and the chip sum — and every one of those
     ...
     * times, and the gate scans about thirty states per configuration. Nothing
     ...
     * that is every `.scroll-region`, plus `.sr-only`.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // it has no SVG of its own — but the rule is the same one that would
      // apply the moment it grows one.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. This lab's three
      // `.xray-details` panels are exactly that case: two ship shut and the
      // third (the group-secret X-ray) only exists once a ceremony has
      // assembled a key. The gate opens each by clicking its <summary> — the
      // route a reader has — rather than setting `.open` from script, which is
      // what the previous gate did to all of them at once, before every scan.
      ...
      // and slides in only on focus. Measuring the parked copy invents a failure for text that is not
      ...
      // the OTHER WCAG-sanctioned visually-hidden idiom, and this page uses it
      // heavily: `dom.ts`'s `mark()` pairs an `aria-hidden` glyph with an
      // `.sr-only` span carrying the same state in words, so every ✓/✗/⚠ on the
      // page has a clipped twin. `.sr-only` is `position: absolute; width: 1px;
      // height: 1px; clip: rect(0 0 0 0)`, which paints nothing at all — the
      // box has a rect and passes `checkVisibility()`, but the compositor draws
      // zero pixels of it. Measuring it reports a ratio for ink that was never
      // laid down: those spans inherit `--bad` from their `.mark-bad` parent
      // and sit over the `.cell-bad` tint, which reads 4.39:1 for text no one
      // can see. This is the same judgement the off-page check above makes, by
      // a different mechanism, and it is deliberately narrow — only a
     ...
     * `aria-hidden` but still painted is skipped by BOTH oracles, so it was
     * checked here rather than assumed. This lab hides three things and no
     * values: the shared header's glyph-free `<svg>`s; the `+` and `=` between
     * the key-assembly chips; and the `✓`/`✗`/`⚠` glyph half of every `mark()`,
     * whose other half is an `.sr-only` span carrying the same state in words —
     * which is precisely why the glyph may be hidden. The glyphs are still
     * painted, in `--ok`/`--bad`/`--warn`, and those three inks are measured for
     * real everywhere else they appear as visible text (`.mark` sets no
     * background of its own, so the surface is identical).
     */
     ...
     *
     * This lab has no SVG figure today, and it builds its DOM with a typed
     * `el()` helper rather than by interpolating strings, so the usual way this
     * mistake arrives — interpolating a string ARRAY into a template literal,
     * which JS joins with commas and leaves as a run of "," text nodes inside
     * a `<g>` — cannot happen here. The guard stays because it is the rule for
     * any SVG anyone adds, and because a walk that measured such a node would
